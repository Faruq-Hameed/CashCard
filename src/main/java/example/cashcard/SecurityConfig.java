...
class SecurityConfig {
    ...
    UserDetailsService userDetailsService(PasswordEncoder passwordEncoder) {
        UserDetails user = User.builder() // build a user for in-memory authentication
                .username("sarah1") // the username is sarah1
                .password(passwordEncoder.encode("abc123")) // password is sarah1Pass encoded using BCrypt
                .roles("USER") // assign the user the role of USER
                .build(); // build the UserDetails object
        return new InMemoryUserDetailsManager(user); // return an in-memory user details manager with the user we just
                                                     // built
        /*
