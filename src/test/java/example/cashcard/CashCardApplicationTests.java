...
// @DirtiesContext(classMode = ClassMode.AFTER_EACH_TEST_METHOD)//ausing Spring
// to start with a clean state, as if those other tests hadn't been run
class CashCardApplicationTests {
	...
	// test with no parameters i.e default values expected to be in th controller
	@Test
	...
	void shouldUpdateAnExistingCashCard() {
		...
		ResponseEntity<Void> response = restTemplate
				.withBasicAuth("sarah1", "abc123")
				// putForEntity() doesn't exist so we use the more general which is exchange()
				// exchange() method is a more general version of the xyzForEntity()
				.exchange("/cashcards/99", HttpMethod.PUT, request, Void.class);
		assertThat(response.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);

		// verify the update took place by retrieving the updated cash card
		ResponseEntity<String> getResponse = restTemplate
				.withBasicAuth("sarah1", "abc123")
				.getForEntity("/cashcards/99", String.class);
		assertThat(getResponse.getStatusCode()).isEqualTo(HttpStatus.OK);
